...
for iter3 in range(num_channels):
    for iter1 in range(number_of_files):
        ...
        channels_dic = RGB_divider(imgs_buffer)
        print(imgs_filename)
        dic_current_channel[imgs_filename] = channels_dic[channels[iter3]]
        #
        num_conduct = 5000
        num_samples = 100 
        r_clt = clt_wjcheon(channels_dic[channels[iter3]],num_conduct,num_samples)
    ...
    for keys, values in dic_current_channel_clt.items(): 
        ...
        counter = counter + 1
    filename_save_hist = 'histogram_of_'+channels[iter3]+'.jpg'
    plt.savefig(filename_save_hist)
    plt.show()
        #%%
    ...
    #%% Save dictionary and Load dictionary 
    import pickle
    ...
    filename_dictionary = 'dic_current_channel_clt' + '_' + channels[iter3]
    save_obj(dic_current_channel_clt,filename_dictionary)
    del dic_current_channel_clt_mean_std
    #loaded_dic = load_obj('dic_current_channel_clt')
    #%%
